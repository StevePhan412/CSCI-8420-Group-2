...
###Repository:### [link to the software's open-source repo — pending Issue #1]
...
CrowdSec Is an Open-source preventative Fail2Ban service that instead of reacting to known malicious behavior and banning appropriately; leverages all that run the program to help stop known malicous IP's. For instance, if someone were to try and bruteforce your machine, fail2ban would detect the brute-force attempt and ban you from trying to access that machine. CrowdSec takes that one step further by recording that IP and distributing it to all other machines running that service. Meaning that attacker would be banned from another machine before even attempting to attack it.
...
162 Commits over 6 months (adv 27/Mth), 224 Open Issues / 1,018 Closed Issues (~22% of all Issues are Open) and they are currently at v1.8.1 (over 223 Releases)

###Popularity:### 

14.8K stars, 715 forks, Depends on Docker, Golang(Go), Grok Patterns, SSH, Sqlite
...
Checkpoint, Cisco, F5, Foritnet, Juniper, Mikrotik OPNsense, PaloAlto, pfSense, Sophos
